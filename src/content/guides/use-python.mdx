...
The browser can enforce `robots.txt` for you. It is off by default, and every
entry point takes browser flags through `args=`:

```python copy
Browser(args=["--obey-robots"])                    # and AsyncBrowser
run_script("scrape.js", args=["--obey-robots"])
```

A request the site disallows then fails rather than being sent: a `Session`
call raises `ToolError`, and `run_script` raises `ScriptError`.

The same flag works on [`CDPServer`](/reference/python#cdpserver) and
[`BiDiServer`](/reference/python#bidiserver), where a blocked request surfaces
as a navigation error in Playwright or Selenium rather than as a Python
exception.

<Callout type="warning" emoji="⚠️">
  The rule applies to **every** request, not just the page you asked for.
  Plenty of sites disallow the directory their own assets live in, so a page
  you are allowed to fetch can load with its scripts and styles blocked, and
  render blank or empty. That is `robots.txt` being honoured, not a failure.
  If a page comes back strangely empty under `--obey-robots`, read the site's
  `robots.txt` before assuming something is broken.
</Callout>
