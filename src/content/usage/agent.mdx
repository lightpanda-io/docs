...
2. **The runtime** A small set of native tools that let's you drive the browser:
   `goto`, `click`, `fill`, `extract`, `evaluate`, `search`, and more.
   Each is available as a slash command (`/goto`, `/click`, ...).
...
Or `OPENAI_API_KEY`, `GOOGLE_API_KEY`, or a local LLM through an Ollama server.
...
You can set explicitly the provider with the CLI option `--provider` or the REPL command `/provider`.
Otherwise the agent will pick one on this order:
...
2. **Auto-detected** - the first key found in priority order
   ...
   `OPENAI_API_KEY`). With several keys on the REPL, you'll be prompted to
   pick.
...
You can set explicitly a model with the CLI option `/model` or the REPL command `/model`.
...
The CLI option `--effot <none|minimal|low|medium|high|xhigh>` or the REPL command `/effort`
...
`/help` list all available slash commands, `/help <command>` for details.
