...
A PandaScript is a web automation script that can be run directly by Lightpanda binary.

No environment to setup like NodeJS or Python, no Puppeteer/Playwright to write, no CDP serialization cost, no LLM required: just vanilla Javascript with a small set of native primitives.
...
Use `goto(...)` to navigate on an URL:
