...
- `page.evaluate(...)` runs JavaScript in the page context, distinct from the
  agent context's own native `eval`. It is not a global; it's a method on the
  page object.
...
- The script's output is whatever it `return`s from the top level (objects
  ...
  `return page.extract({ ... });` or `return results;`. A bare trailing expression
...
`page.evaluate(...)` still returns the page evaluate tool result text. When it
...
once it reaches `waitUntil` (`load` by default), and rejects if navigation
fails or times out (`"navigation timed out"`). Pass `waitUntil: "networkidle"`
(or another state) to wait past `load` directly, or follow with
`waitForState(...)` / `waitForSelector(...)` when completeness matters.
...
`page.evaluate(...)` is the explicit escape hatch into the current page's JavaScript
...
`page.evaluate(...)` cannot call `page.goto`, `page.extract`, or other agent
primitives. Agent scripts cannot access `document` directly. If you need page
DOM data, prefer `page.extract(...)`; use `page.evaluate(...)` only for page
behavior that extraction cannot express.

`page.waitForScript(...)` also evaluates in the page context, repeatedly, until the
