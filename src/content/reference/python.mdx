...
[`Browser()`](/reference/python-api#browser) spawns the bundled binary when constructed. It is not fork-inheritable: create a fresh instance in a forked child.
...
[`AsyncBrowser`](/reference/python-api#asyncbrowser) mirrors `Browser` for asyncio: every call runs on a browser-owned thread pool, so the event loop is never blocked.
...
`Browser.new_session()` and `AsyncBrowser.new_session()` are the only way to obtain a [`Session`](/reference/python-api#session) or [`AsyncSession`](/reference/python-api#asyncsession); do not construct one directly.
...
| Error | Raised when |
|---|---|
| [`LightpandaError`](/reference/python-api#lightpandaerror) | Base class for every error the package raises. |
| [`ProtocolError`](/reference/python-api#protocolerror) | The connection to the browser process failed: a malformed request, a timeout, or an internal error. Carries a `code` attribute. |
| [`ToolError`](/reference/python-api#toolerror) | A browser action reported failure, such as a bad selector, a JS exception inside `evaluate`, or a call on a closed session. |
| [`ScriptError`](/reference/python-api#scripterror) | `run_script` or `run_script_async` exited with a non-zero status, or the script file doesn't exist (`returncode=-1`). Carries `returncode`, `stdout`, and `stderr` attributes. |
