...
| `page.screenshot` | `screenshot({ path, selector?, backendNodeId?, fullPage?, strip?, url?, timeout? })`, never positional | `Saved <width>x<height> PNG to <path>` | `path` is required in scripts: a script cannot display an inline image, so omitting it throws `pass \`path\`: this client cannot display an inline image`. Renders the page, or one scoped node, as a PNG: the text layout Lightpanda computes, not a pixel-accurate rendering (no images, fonts, or CSS colours). `strip` is an object of element groups to omit: `js`, `css`, `ui`, `invisible`, `shell`, `clutter`. |
...
| `page.scroll` | `scroll({ selector?, x, y })` — `selector` scrolls that element's scroll container instead of the window; an omitted axis keeps its current offset, it is not reset to `0` | `Scrolled window to x: <x>, y: <y>` |
...
- A `null` positional omits that argument: `press(null, "Enter")` presses on `document.activeElement` — the focused element, falling back to `<body>` when nothing is explicitly focused.
