...
The result comes back as `{"result":{"content":[{"type":"text","text":"..."}],"isError":false}}`. `goto` returns an error directly on a genuine navigation failure (e.g. `navigation failed: CouldntResolveHost`); a timeout instead reports `"Navigation started but the page did not finish loading before the timeout."`, and an HTTP error page (404/500) still reports `"Navigated successfully."` (see [Known behaviors](/usage/mcp#known-behaviors)).
...
| Name | Arguments | Description |
|---|---|---|
| `markdown` | `selector?`, `backendNodeId?`, `maxBytes?`, `strip?`, `url?`, `timeout?` | Render the page, or a subtree, as markdown. Scope with `selector` or `backendNodeId` to read just the relevant region: full-page markdown is the last resort. Use `maxBytes` to cap long pages. `strip` takes the same element groups as the `html` tool. |
...
| `nodeDetails` | `backendNodeId` | Tag, role, name, interactivity, disabled, value, input type, placeholder, href, id, class, checked, and select options for a node, plus a ready-to-use CSS `selector` that resolves to the node (the first match, as click/fill resolve it). The canonical way to turn a tree backendNodeId into a CSS selector. |
| `findElement` | `role?`, `name?` | Find interactive elements by role and/or accessible name. `name` matches case-insensitively as a substring, or as a JavaScript regex when written `/regex/` (unanchored; flags `i`, `m`, `s`, `u` accepted; case-insensitive even without `i`, prefix `(?-i)` to make it case-sensitive). Returns matching elements with their backend node IDs. Useful for locating specific elements without parsing the full semantic tree. |
| `interactiveElements` | `url?`, `timeout?` | List every visible interactive element as JSON: native controls, ARIA widgets, contenteditable regions, elements with event listeners, and focusable elements. Each entry has `backendNodeId`, `tagName`, `role`, `name`, `type` (why it counts as interactive), `tabIndex`, and when present `listeners`, `disabled`, `id`, `class`, `href`, `inputType`, `value`, `elementName` and `placeholder`. `findElement` is cheaper when you only need to locate one by role or name. |
| `structuredData` | `url?`, `timeout?` | Page metadata as JSON: `jsonLd` (each JSON-LD block as a string), `openGraph`, `twitterCard`, `meta` and `links` (key/value lists), plus `alternate` (hreflang variants) and `linkHeaders` (relations from the HTTP Link header) when present. Empty sections come back as empty arrays. |
| `detectForms` | `url?`, `timeout?` | List the forms on the page as JSON: each form's `backendNodeId`, `action`, `method` and `fields`, where each field has `backendNodeId`, `tagName`, `name`, `inputType`, `required`, `disabled`, and when present `value`, `placeholder` and select `options`. Returns no CSS selectors; get one per field with `nodeDetails` so fill calls stay replayable. |
...
| `scroll` | `selector?` / `backendNodeId?`, `x?`, `y?` | Scroll the page, or an element's scroll container if `selector` or `backendNodeId` is given (`selector` preferred, as with click/fill). If that element isn't itself a scroll container, its nearest scrollable ancestor scrolls instead, and the result reports whichever node actually moved. An omitted axis keeps its current offset. Returns the resulting scroll position and the current page URL and title. |
