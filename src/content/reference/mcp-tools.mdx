...
| `scroll` | `backendNodeId?`, `x?`, `y?` | Scroll the page, or a specific element if `backendNodeId` is given. If that element isn't itself a scroll container, its nearest scrollable ancestor scrolls instead, and the result reports whichever node actually moved. Returns the resulting scroll position and the current page URL and title. |
