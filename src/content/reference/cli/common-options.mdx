...
Every Lightpanda command ([fetch](/reference/cli/fetch), [serve](/reference/cli/serve), [mcp](/reference/cli/mcp), [agent](/reference/cli/agent), [run](/reference/cli#run)) accepts the following options in addition to its own.
  ...
  --experimental-features <FEATURE>
          ...
          Allowed values:
              cors          Obey CORS (cross-origin resource sharing) checks
                            ...
                            unconditionally.
              serviceworker Enable the Service Worker API (navigator.serviceWorker
                            and registration), hidden by default.
