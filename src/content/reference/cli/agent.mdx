...
Options:
  ...
  --provider <PROVIDER>
          ...
          MISTRAL_API_KEY, VERTEX_API_KEY, or GOOGLE_CLOUD_PROJECT via
          gcloud). With exactly one key set: that provider is used. With
          multiple keys on a TTY: you'll be prompted to pick; in
          non-interactive contexts, pass --provider explicitly. With no
          keys set: falls back to the basic REPL (slash commands only, no
          natural-language input, no LOGIN / ACCEPT_COOKIES keywords).
