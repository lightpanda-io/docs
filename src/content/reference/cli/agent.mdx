...
Arguments:
[SCRIPT]
          Optional path to a .js script, or `-` to read the script from
          stdin. Runs the script (no LLM calls) and exits; `lightpanda run
          SCRIPT` is the preferred spelling. With no script and no --task,
          the REPL starts; from there /load runs a script and /save exports
...
Options:
  ...
  --provider <PROVIDER>
          ...
          MISTRAL_API_KEY, OPENROUTER_API_KEY, ORCAROUTER_API_KEY,
          VERTEX_API_KEY). With exactly one key set:
          ...
          --provider llama_cpp. Same for codex, which uses a ChatGPT
          subscription (OAuth) instead of an API key.

          Allowed values: "anthropic", "openai", "gemini", "huggingface",
                          "vercel", "mistral", "openrouter", "orcarouter",
                          "ollama", "llama_cpp", "vertex", "codex",
                          "openai_compatible".
  ...
  --search-engine <ENGINE>
          Which web search API the `search` tool uses. Default: auto,
          which takes the first with a key in the environment, in the
          order brave, tavily, exa, keenable. `keenable` also works with
          no key at all, through a rate-limited public endpoint.
          Allowed values: auto, brave, tavily, exa, keenable.
  ...
  --url <URL>
          Open this page before the first turn. With --task it saves the
          model a turn spent navigating; with --save the opening
          navigation is the first line of the script.
  --verbosity <LEVEL>
...
API keys are read from the environment: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_API_KEY`/`GEMINI_API_KEY`, `HF_TOKEN`, `AI_GATEWAY_API_KEY`, `MISTRAL_API_KEY`, `OPENROUTER_API_KEY`, or `ORCAROUTER_API_KEY`. Any other OpenAI-compatible server is used through `OPENAI_BASE_URL` + `OPENAI_API_KEY`. The local servers (Ollama, llama.cpp) do not require an API key.
...
lightpanda agent --provider openrouter --model anthropic/claude-sonnet-5
